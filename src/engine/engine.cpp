...
namespace CityFlow {
    ...
    void Engine::vehicleControl(Vehicle &vehicle, std::vector<std::pair<Vehicle *, double>> &buffer) {
        ...
        double acceleration = nextSpeed - speed;
...
//                    assert(vehicle->getPartner() == nullptr);
    ...
    std::map<std::string, double>  Engine::getLaneVehicleEmission() const{
        std::map<std::string, double> ret;
        double scale=742;
        for (const Lane *lane : roadnet.getLanes()) {
            double LaneEmission = 0;
            for (Vehicle *vehicle : lane->getVehicles()) {
                double v = vehicle->getSpeed();
                double acc= vehicle->getAcceleration();
                double emission = (7192 + 1022 * acc * v + 0.0 * acc * acc * v + -161 * v + 23.33 * v * v + 0.0 * v * v * v) / scale;
                LaneEmission += std::max(emission, (double) 0.);
            }
            ret.emplace(lane->getId(), LaneEmission);
        }
        return ret;
    }
    
    std::map<std::string, double>  Engine::getLaneVehicleDelay() const{
        std::map<std::string, double> ret;
        for (const Lane *lane : roadnet.getLanes()) {
            double LaneDelay = 0;
            for (Vehicle *vehicle : lane->getVehicles()) {
                double v = vehicle->getSpeed();
                double maxv= vehicle->getMaxSpeed();
                double delay = (1-(v/maxv));
                LaneDelay += std::max(delay, (double) 0.);
            }
            ret.emplace(lane->getId(), LaneDelay);
        }
        return ret;
    }
    ...
    std::map<std::string, double> Engine::getVehicleAcceleration() const {
        std::map<std::string, double> ret;
        for (const Vehicle* vehicle : getRunningVehicles()) {
            ret.emplace(vehicle->getId(), vehicle->getAcceleration());
        }
        return ret;
    }
    ...
    double Engine::getVehicleEmission() const {
        float TotalEmission=0;
        double scale=742;
        for (const Vehicle* vehicle : getRunningVehicles()) {
            double v = vehicle->getSpeed();
            double acc= vehicle->getAcceleration();
            float emission = (7192 + 1022 * acc * v + 0.0 * acc * acc * v + -161 * v + 23.33 * v * v + 0.0 * v * v * v) / scale;
            TotalEmission += std::max(emission, (float) 0.);
        }
