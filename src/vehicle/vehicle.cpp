...
namespace CityFlow {
    ...
    void Vehicle::setAcceleration(double acc) {
        ...
        acc = min2double(acc, vehicleInfo.maxPosAcc);
        acc = max2double(acc, -vehicleInfo.maxNegAcc);
        
        buffer.acceleration = acc; 
        buffer.isAccelerationSet = true;
    ...
    void Vehicle::update() { // TODO: use something like reflection?
        ...
        if (buffer.isAccelerationSet) {
            vehicleInfo.acceleration = buffer.acceleration;            
            buffer.isAccelerationSet = false;
        }
    ...
    std::map<std::string, std::string> Vehicle::getInfo() const{
        ...
        info["acceleration"] = std::to_string(getAcceleration());
